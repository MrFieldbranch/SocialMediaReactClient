...
export class SocialMediaApiService {
  ...
  async getInterestsNotOwnedByMeAsync(): Promise<IInterestResponse[]> {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(
        `Failed to fetch the interests that you don't have: ${
          errorMessage || "No error message provided by the server."
        }`
      );
    }
  ...
  async getStrangersAsync(): Promise<IBasicUserResponse[]> {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(
        `Failed to fetch people that are not your friends: ${
          errorMessage || "No error message provided by the server."
        }`
      );
    }
  ...
  > {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(
        `Failed to fetch people that are not your friends, based on common interests: ${
          errorMessage || "No error message provided by the server."
        }`
      );
    }
  ...
  > {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(
        `Failed to fetch people who have pending friend requests to you: ${
          errorMessage || "No error message provided by the server."
        }`
      );
    }
  ...
  > {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(
        `Failed to fetch people who have pending friend requests from you: ${
          errorMessage || "No error message provided by the server."
        }`
      );
    }
