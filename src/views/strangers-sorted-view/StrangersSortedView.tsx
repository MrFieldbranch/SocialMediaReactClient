...
import BasicUserSharedInterests from '../../components/basic-user-shared-interests/BasicUserSharedInterests';
...
const StrangersSortedView = () => {
	...
	useEffect(() => {
		...
		const fetchStrangersSorted = async () => {
			try {
				const strangersSortedFromApi = await socialMediaApiService.getStrangersBasedOnInterestsAsync();
				if (isMounted) {
